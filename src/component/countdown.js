...
var MARGIN_TOP = 60;
var MARGIN_LEFT = 30;
const endTime = new Date(2017,4,13,6,0,0);
...
function updateBalls(){
	for(var i=0;i<balls.length;i++){
		balls[i].x += balls[i].vx;  //小球x轴的位置
		balls[i].y += balls[i].vy;  //小球y轴的位置
		balls[i].vy += balls[i].g;  //小球加速的
		if(balls[i].y>= WINDOW_HEIGHT-RADIUS){  //碰撞检测（屏幕高度-小球半径）
			balls[i].y = WINDOW_HEIGHT-RADIUS;
			balls[i].vy = -balls[i].vy*0.65; //小球的减速度
		}
	}
}
		...
		for(var j=0;j<digit[num][i].length;j++)
			if(digit[num][i][j] == 1){
				var aBall = {
					...
					g:1.5+Math.random(),
					vx:Math.pow(-1,Math.ceil(Math.random()*1000))*4, //小球x方向速度
					vy:-5, //小球Y方向速度
